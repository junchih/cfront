 ...
 */
/**
ncsldiff - report number of changed source lines in a file

Synopsis:	ncsldiff  oldfile  newfile

Compares the two files,
and reports on the standard output the number of lines in the
current file, lines in the old file, the difference of the two,
the number of deleted or changed lines, the number of new or changed lines,
and the number of unchanged lines.

The changes from diff(1) are: modifying output() to report counts
instead of printing changed lines.  Also e, f, and h options deleted.
A c option was added to suppress printing of headers.
NOTE: IBM version compiled with -DIBM370 does not have capability
to specify directories in place of file names and does not
allow input from a terminal.

ncsldiff.c 1.2 of 10/16/82 this modification by G. Patton
*/

static char	*sccs = "@(#)ncsldiff.c	1.2";
...
#include <stdio.h>
#if IBM370
/* signal and stat are not used in the IBM version.  sys/types.h is
  used by sys/stat.h and is not required.    
  ctype.h has been included in stdio.h on the ibm machine.  */
#else
#include <ctype.h>
#include <sys/types.h>
#include <sys/stat.h>
...
int ncom = 0;   /* -1,0 = -c,normal = noheadings,headings */
int status = 2;
int anychange = 0;
char *empty = "";
int bflag;
...
{
#if IBM370
/* unlink of tempfile not done for IBM implementation */
#else
	if(tempfile)
		unlink(tempfile);
#endif
...
char *p;
{
	...
	char *realloc();
#if IBM370
/* freeing memory is not done in ibm implementation */
#else
	free(p);
	free(dummy);
	dummy = malloc(1);
...
#if IBM370
/* the IBM implementation does not allow only specifying directories */
#else
filename(pa1, pa2)
char **pa1, **pa2;
{
...
char **argv;
{
	...
	if(argc>3 && *argv[1]=='-') {
		...
		for(k=1;argv[0][k];k++) {
			switch(argv[0][k]) {
			case 'e':
				mesg("e option invalid",empty);
				done();
			case 'f':
				mesg("f option invalid",empty);
				done();
			case 'b':
				bflag = 1;
				break;
			case 'h':
				mesg("h option invalid",empty);
				done();
			case 'c':
				ncom = -1;  /* no headings */
				break;
...
#if IBM370
/* the IBM implementation does not allow only specifying directories
   or reading input from a terminal */
#else
...
char **argv;
{
	...
	if(m==0)
		change(1,0,1,len[1]);
	unchanged = len[0] - Deleted;
	Delta = len[1] - len[0];
	if( ncom == 0 ) {
		printf("Count	Count	Size	Del.	New    	\n");
		printf("Curr.	Prev.	Delta	or Chg.	or Chg.	Unchg.\n");
	}
	printf("%-8d%-8d%-8d%-8d%-8d%-8d\n",
		len[1], len[0], Delta, Deleted, Added, unchanged);
}
